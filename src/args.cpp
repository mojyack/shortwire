...
auto Args::parse(const int argc, const char* const argv[]) -> std::optional<Args> {
    ...
    parser.kwarg(&args.peer_linker_user_cert_path, {"--peer-linker-cert"}, {"FILE", "user certificate for peer-linker", args::State::Initialized});
    parser.kwarg(&args.key_file, {"--key"}, {"FILE", "enable encryption using the key", args::State::Initialized});
